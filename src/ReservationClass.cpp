...
void ReservationClass::SaveReservationToFile()
{
    if(Reserve.empty()==false)
    {
        ...
        try
        {
            ...
            for(auto Reserve_info:Reserve)
            {
                file_write << Reserve_info.second.busno << "\t"<< Reserve_info.second.ticket_id <<"\t" << Reserve_info.second.User_info.idtype <<"\t"  << Reserve_info.second.User_info.name << "\t"<< Reserve_info.second.User_info.age << "\t" << Reserve_info.second.User_info.gender <<"\t" <<Reserve_info.second.User_info.mobileno;
...
bool ReservationClass::SaveUserBookingstoUserFile(string mobile_no,ReservationDetails User_info)
{
    ...
    try
    {
        ...
        file_write << User_info.busno << "\t"<< User_info.ticket_id ;
...
UserDetails ReservationClass::RegisterUser(string mobile_no)
{
    ...
    //Users_map.insert(pair<string,UserDetails>(mobile_no,User_info));
...
string ReservationClass::BookTicket(UserDetails User_info)
{
    ...
    if(matched_buses!=0)
    {
        ...
        //cout << "Boarding/Dropoff Points:" << endl;
        ...
        for(int i=0;i<count;i++)
        {
            ...
            {
                if(is_Female_Reservation&&!is_Female&&stoi(seatno)>0&&stoi(seatno)<=8) // comment for 0 to 8 use commnets wherever possible
                {
        ...
        sort(passenger_obj.seats_booked.begin(),passenger_obj.seats_booked.end());
...
bool ReservationClass::ShowUserBookings(string mobile_no)
{
    ...
    try
    {
        ...
        for(int i=0;i<user_info.tickets_booked;i++)
        {
            ...
            cout  << "\nBus No: " << obj.busno << "\tTicket Id: "<< obj.ticket_id  << endl;
